...
| `rusty_rtos_alloc` on the four bare-metal targets: `--no-default-features` and `--features small-metal`, under `RUSTFLAGS="--cfg ra_single_threaded --cfg ra_small_profile"` | all 8 rungs pass (2026-09-09) | the seam's firmware half: rusty_alloc 2.0.4's own opt-ins; without `ra_single_threaded` it refuses to compile, by design (verified); `std` + `small-metal` on the host stays inert (verified). No board has run it: K4's row |
